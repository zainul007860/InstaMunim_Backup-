...
function OrderPageContent() {
  ...
  // Order & Payment State
  const [step, setStep] = useState<"menu" | "checkout" | "waiting_verification" | "success">("menu");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [placedOrderId, setPlacedOrderId] = useState<string>("");
  const [activePendingSaleId, setActivePendingSaleId] = useState<string>("");
  const [waitingCountdown, setWaitingCountdown] = useState<number>(180); // 3 mins auto-timer

  useEffect(() => {
    if (tableParam) setTableNumber(tableParam);
  }, [tableParam]);

  // Realtime & Polling listener for automatic bank confirmation
  useEffect(() => {
    if (!activePendingSaleId || step !== "waiting_verification") return;

    // 1. Supabase Realtime Channel Listener
    const channel = supabase
      .channel(`sale-verify-${activePendingSaleId}`)
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'sales', filter: `id=eq.${activePendingSaleId}` },
        (payload: any) => {
          if (payload.new && (payload.new.payment_type?.includes("PAID") || payload.new.payment_type === "Table QR UPI" || payload.new.payment_type?.includes("VERIFIED"))) {
            setStep("success");
          }
        }
      )
    ...
    // 2. Fallback Fast Polling every 2.5 seconds (in case realtime is delayed)
    const pollInterval = setInterval(async () => {
      try {
        const { data, error } = await supabase
          .from("sales")
          .select("id, payment_type")
          .eq("id", activePendingSaleId)
          .single();

        if (data && (data.payment_type?.includes("PAID") || data.payment_type === "Table QR UPI" || data.payment_type?.includes("VERIFIED"))) {
          setStep("success");
        }
      } catch (e) {
        // silent
  ...
  // Handle Order Placement & Live Auto-Verification Initiation
  const handleInitiateOrderPayment = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        alert(`Failed to initiate order: ${error.message || "Database error"}`);
        setIsSubmitting(false);
        return;
      }

      const newId = data?.[0]?.id || `ORD-${Date.now().toString().slice(-4)}`;
      setPlacedOrderId(newId);
      setActivePendingSaleId(newId);
  ...
  // Direct manual mark as verified by customer if they uploaded or counter verified
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center p-6 text-center text-white space-y-4">
        ...
        <p className="text-xs text-zinc-400 max-w-xs leading-relaxed">
          Invalid QR Code or store is currently inactive. Please contact the counter staff.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-zinc-900 font-sans antialiased overflow-y-auto selection:bg-orange-500 selection:text-white">
      ...
      {step === "checkout" && (
        <main className="max-w-md mx-auto p-4 pb-28 space-y-4 animate-in fade-in slide-in-from-bottom-4">
          ...
          <div className="rounded-3xl bg-gradient-to-b from-zinc-900 to-zinc-950 text-white p-5 space-y-4 shadow-2xl border border-zinc-800">
            ...
            <Button
              onClick={handleInitiateOrderPayment}
            ...
            >
              {isSubmitting ? (
                <>
                  <Loader2 className="h-5 w-5 animate-spin" /> Verifying Order Details...
                </>
              ) : (
                <>
                  <CheckCircle2 className="h-5 w-5" /> I Have Paid ₹{totalAmount} 🚀
                </>
              )}
            </Button>
          </div>
      ...
      {/* 5. STEP: WAITING FOR BANK SMS / POS AUTO-CONFIRMATION RADAR */}
      {step === "waiting_verification" && (
        <main className="max-w-md mx-auto p-5 text-center space-y-5 animate-in zoom-in-95 pt-8 pb-32">
          
          {/* Animated Bank Radar Pulse */}
          <div className="relative w-36 h-36 mx-auto flex items-center justify-center">
            <div className="absolute inset-0 rounded-full bg-emerald-500/10 animate-ping" />
            <div className="absolute inset-3 rounded-full bg-emerald-500/20 animate-pulse" />
            <div className="w-24 h-24 rounded-3xl bg-gradient-to-tr from-zinc-900 via-zinc-950 to-black text-emerald-400 flex flex-col items-center justify-center shadow-2xl border border-emerald-500/40 z-10">
              <RefreshCw className="h-8 w-8 animate-spin text-emerald-400" />
              <span className="text-[10px] font-mono font-black text-emerald-300 mt-1">
                {Math.floor(waitingCountdown / 60)}:{String(waitingCountdown % 60).padStart(2, '0')}
              </span>
            </div>
          </div>

          <div className="space-y-2">
            <Badge className="bg-amber-500/10 text-amber-500 border border-amber-500/30 text-[10px] font-black uppercase tracking-widest px-3 py-1">
              ⏳ AWAITING CASHIER CONFIRMATION
            </Badge>
            <h2 className="text-xl font-black uppercase tracking-tight text-zinc-900">
              Payment Initiated for Table #{tableNumber}
            </h2>
            <div className="p-3.5 bg-amber-50 rounded-2xl border border-amber-200/80 text-left space-y-1">
              <p className="text-xs font-black text-amber-900 flex items-center gap-1.5">
                <span>📍</span> Go & Confirm at Cash Counter
              </p>
              <p className="text-[11px] text-amber-800 font-medium leading-relaxed">
                After completing your payment, please inform the cashier at the counter. Once confirmed by the cashier, this screen will instantly update to <strong>Order Confirmed & Thank You</strong>!
              </p>
            </div>
          </div>

          {/* Live Sync Status Pill */}
          <div className="rounded-2xl bg-white border border-zinc-200/90 p-4 space-y-2.5 text-left text-xs shadow-sm">
            <div className="flex items-center justify-between text-zinc-400 text-[10px] font-black uppercase tracking-widest">
              <span>Target Table</span>
              <span className="text-orange-600 font-black">TABLE #{tableNumber}</span>
            </div>
            ...
            <div className="p-2.5 rounded-xl bg-zinc-50 border border-zinc-200/80 flex items-center gap-2 text-[11px] text-zinc-600 font-medium">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping shrink-0" />
              <span>Realtime Connected to Counter POS Dashboard...</span>
            </div>
          </div>
          ...
          <div className="space-y-3 pt-2 pb-6">
            <Button
              onClick={handleManualMarkDone}
              className="w-full h-13 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black text-xs uppercase tracking-wider rounded-2xl shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 border-0"
            >
              <CheckCircle2 className="h-4 w-4" /> Cashier Confirmed? Show Success Screen
            </Button>
            ...
            <div>
              <button
                ...
                className="text-xs font-black text-zinc-500 uppercase tracking-wider hover:text-zinc-900 transition-colors py-2 px-4"
              >
      ...
      {/* 6. STEP 3: ORDER SUCCESS (THANK YOU SCREEN) */}
      {step === "success" && (
        <main className="max-w-md mx-auto p-6 text-center space-y-6 animate-in zoom-in-95 pt-12">
          <div className="relative inline-block">
            ...
            <div className="absolute -bottom-1 -right-1 bg-zinc-950 text-white text-[10px] font-black uppercase px-2.5 py-1 rounded-full border border-zinc-800 shadow-md">
              TABLE #{tableNumber}
            </div>
          </div>

          <div className="space-y-1.5">
            <h2 className="text-2xl font-black uppercase tracking-tight text-zinc-900">Thank You! 🍽️</h2>
            <p className="text-xs font-bold text-zinc-600 leading-relaxed max-w-xs mx-auto">
              Your order is confirmed. Please relax at <strong>Table #{tableNumber}</strong> while your meal is being prepared!
            </p>
          </div>
